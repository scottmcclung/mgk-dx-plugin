        ...
        const stream = fs
          ...
          .on('end', () => {
            ...
            uniqueObjects.forEach((objectName) => {
              ...
              expect(
                isCustomObject,
                `Object '${objectName}' should have custom naming extension when --customobjectsonly flag is used`,
              ).to.be.true;
            });
          });
  ...
  describe('Error Handling', () => {
    it('should handle invalid org gracefully', () => {
      try {
        ...
        expect(error.message).to.include('No AuthInfo found');
      }
    });
