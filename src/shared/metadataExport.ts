...
export default class MetadataExport {
    ...
    public async getExport() {
        ...
        if (this._sobjectMetadata.size > 0) {
            ...
            await this.getDescribeMetadata(
                Array.from(this._sobjectMetadata.keys()),
            );
        }
    ...
    protected async loadEntityDefinitions(sobjectNames: string[]) {
        const sobjectFilter =
            sobjectNames.length > 0
                ? `AND QualifiedApiName IN (${sobjectNames.map((name) => `'${name}'`).join(",")})`
                : "";
        const strSoql = `
            ...
            `;
        const entities: QueryResult<EntityDefinition> = await this._org
            .getConnection()
            .query(strSoql);
        if (entities.records) {
            for (const object of entities.records) {
                if (
                    this._customObjectsOnly &&
                    !object.QualifiedApiName.endsWith("__c")
                )
                    continue;
                this._sobjectMetadata.set(object.QualifiedApiName, {
                    ...
                    RecordTypesSupported:
                        object.RecordTypesSupported.recordTypeInfos || [],
    ...
    protected async loadCustomObjectMetadata() {
        const metadataList: FileProperties[] = await this._org
            .getConnection()
            .metadata.list(
                [
                    {
        ...
        if (metadataList) {
            for (const object of metadataList) {
                if (
                    !object.fullName ||
                    !this._sobjectMetadata.has(object.fullName)
                )
                    continue;
     ...
     * @protected
     */
    protected async getDescribeMetadata(
        sobjectNames: string[]
        ...
        for (const object of metadata) {
            ...
            const fieldDescribeMap = this.getFieldDescribeMap(
                object.fields,
                object.name
            );
            ...
            const fieldDefinitionMap = await this.getFieldDefinitionMapByObject(
                object.name
            );
            ...
            const objectDefinition = Object.assign(
                this._sobjectMetadata.get(object.name),
                {
                    fields: fieldMap,
    ...
    protected getFieldDescribeMap(fields: Field[], objectName: string) {
        ...
        for (const field of fields) {
            fieldDescribeMap.set(
                field.name,
                this.normalizeFieldMetadata(field, objectName),
            );
        }
        ...
        if (fieldDefinitions) {
            fieldDefinitions.records.forEach((field) => {
                fieldDefinitionMap.set(field.QualifiedApiName, {
                    ...
                    dataOwner: field.BusinessOwner
                        ? field.BusinessOwner.Name
                        : "",
    ...
    protected formattedDataType(field: Field): string {
        if (field.type === "reference") {
            ...
            return (
                translatedFieldTypes[field.type] + "(" + field.digits + ",0)"
            );
        } else {
